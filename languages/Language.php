...
class Language
{
    ...
    public $mVariants;
    public $mCode;
    public $mLoaded = false;
    public $mMagicExtensions = [];
    public $mMagicHookDone = false;
     ...
     */
    private static $lre = "\xE2\x80\xAA"; // U+202A LEFT-TO-RIGHT EMBEDDING
    private static $rle = "\xE2\x80\xAB"; // U+202B RIGHT-TO-LEFT EMBEDDING
    private static $pdf = "\xE2\x80\xAC"; // U+202C POP DIRECTIONAL FORMATTING
...
$trace = debug_backtrace();
     ...
     * @return string
     */
    public function getUserToggle($tog)
    {
$trace = debug_backtrace();
	  error_log(__FILE__);
	  var_dump(__FUNCTION__);
     ...
     * @return string
     */
    public function getIranianCalendarMonthName($key)
    {
     ...
     * @return string
     */
    public function getHebrewCalendarMonthName($key)
    {
     ...
     * @return string
     */
    public function getHebrewCalendarMonthNameGen($key)
    {
     ...
     * @return string
     */
    public function getHijriCalendarMonthName($key)
    {
...
$trace = debug_backtrace();
     ...
     * @return mixed|string
     */
    public function ucwordbreaksCallbackAscii($matches)
    {
$trace = debug_backtrace();
	  error_log(__FILE__);
	  var_dump(__FUNCTION__);
     ...
     * @return string
     */
    public function ucwordbreaksCallbackMB($matches)
    {
$trace = debug_backtrace();
	  error_log(__FILE__);
	  var_dump(__FUNCTION__);
     ...
     * @return string
     */
    public function ucwordsCallbackMB($matches)
    {
$trace = debug_backtrace();
	  error_log(__FILE__);
	  var_dump(__FUNCTION__);
...
$trace = debug_backtrace();
     ...
     * @deprecated No-op since 1.28
     */
    public function recodeForEdit($s)
    {
$trace = debug_backtrace();
	  error_log(__FILE__);
	  var_dump(__FUNCTION__);
...
$trace = debug_backtrace();
     ...
     * @return string
     */
    public function transformUsingPairFile($file, $string)
    {
     ...
     */
    public function addMagicWordsByLang($newWords)
    {
$trace = debug_backtrace();
	  error_log(__FILE__);
	  var_dump(__FUNCTION__);
...
$trace = debug_backtrace();
     ...
     * languages, some such as Bengali (bn) want �?,৯৩,২৯�?.২৩�? and others such as
...
$trace = debug_backtrace();
     ...
     * @return string
     */
    public function listToText(array $l)
    {
        $m = count($l) - 1;
        if ($m < 0) {
            return '';
...
$trace = debug_backtrace();
     ...
     * @return string
     */
    public function truncate($string, $length, $ellipsis = '...', $adjustLength = true)
    {
...
$trace = debug_backtrace();
     ...
     * @return mixed
     */
    public function fixVariableInNamespace($talk)
    {
        ...
        $talk = preg_replace_callback(
            '/{{grammar:(.*?)\|(.*?)}}/i',
            [ &$this, 'replaceGrammarInNamespace' ],
            $talk
        );
     ...
     * @return string
     */
    public function replaceGrammarInNamespace($m)
    {
$trace = debug_backtrace();
	  error_log(__FILE__);
	  var_dump(__FUNCTION__);
    ...
    public function formatTimePeriod($seconds, $format = [])
    {
        ...
        $secondsMsg = wfMessage(
            $format['noabbrevs'] ? 'seconds' : 'seconds-abbrev'
        )->inLanguage($this);
        $minutesMsg = wfMessage(
            $format['noabbrevs'] ? 'minutes' : 'minutes-abbrev'
        )->inLanguage($this);
        $hoursMsg = wfMessage(
            $format['noabbrevs'] ? 'hours' : 'hours-abbrev'
        )->inLanguage($this);
        $daysMsg = wfMessage(
            $format['noabbrevs'] ? 'days' : 'days-abbrev'
        )->inLanguage($this);
