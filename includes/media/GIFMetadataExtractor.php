...
class GIFMetadataExtractor
{
     ...
     * @return void
     */
    public static function readGCT($fh, $bpp)
    {
        if ($bpp > 0) {
            $max = pow(2, $bpp);
            for ($i = 1; $i <= $max; ++$i) {
     ...
     * @return int
     */
    public static function decodeBPP($data)
    {
        if (strlen($data) < 1) {
            throw new Exception("Ran out of input");
        }
     ...
     * @throws Exception
     */
    public static function skipBlock($fh)
    {
        while (!feof($fh)) {
            $buf = fread($fh, 1);
            if (strlen($buf) < 1) {
     ...
     */
    public static function readBlock($fh, $includeLengths = false)
    {
        $data = '';
