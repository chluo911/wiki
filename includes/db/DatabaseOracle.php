...
class ORAResult
{
     ...
     * @param bool $unique
     */
    public function __construct(&$db, $stmt, $unique = false)
    {
        $this->db =& $db;
...
$trace = debug_backtrace();
    ...
    public function queryIgnore($sql, $fname = '')
    {
$trace = debug_backtrace();
	  error_log(__FILE__);
	  var_dump(__FUNCTION__);
...
$trace = debug_backtrace();
    ...
    public function fieldName($stmt, $n)
    {
$trace = debug_backtrace();
	  error_log(__FILE__);
	  var_dump(__FUNCTION__);
...
$trace = debug_backtrace();
    ...
    public function tableNameInternal($name)
    {
    ...
    public function delete($table, $conds, $fname = __METHOD__)
    {
        ...
        if ($table == $this->tableName('user')) {
            $this->update(
                'archive',
                [ 'ar_user' => 0 ],
                [ 'ar_user' => $conds['user_id'] ],
                $fname
            );
            $this->update(
                'ipblocks',
                [ 'ipb_user' => 0 ],
                [ 'ipb_user' => $conds['user_id'] ],
                $fname
            );
            $this->update(
                'image',
                [ 'img_user' => 0 ],
                [ 'img_user' => $conds['user_id'] ],
                $fname
            );
            $this->update(
                'oldimage',
                [ 'oi_user' => 0 ],
                [ 'oi_user' => $conds['user_id'] ],
                $fname
            );
            $this->update(
                'filearchive',
                [ 'fa_deleted_user' => 0 ],
                [ 'fa_deleted_user' => $conds['user_id'] ],
                $fname
            );
            $this->update(
                'filearchive',
                [ 'fa_user' => 0 ],
                [ 'fa_user' => $conds['user_id'] ],
                $fname
            );
            $this->update(
                'uploadstash',
                [ 'us_user' => 0 ],
                [ 'us_user' => $conds['user_id'] ],
                $fname
            );
            $this->update(
                'recentchanges',
                [ 'rc_user' => 0 ],
                [ 'rc_user' => $conds['user_id'] ],
                $fname
            );
            $this->update(
                'logging',
