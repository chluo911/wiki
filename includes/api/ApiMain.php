        ...
        try {
            ...
            if ($this->mModule->isWriteMode() && $this->getRequest()->wasPosted()) {
                $this->getStats()->timing(
                    'api.' . $this->mModule->getModuleName() . '.executeTiming',
                    1000 * $runTime
                );
            }
        } catch (Exception $e) {
...
$trace = debug_backtrace();
    ...
    public function modifyHelp(array &$help, array $options, array &$tocData)
    {
        ...
        // Fill 'permissions'
        $help['permissions'] .= Html::openElement(
            'div',
            [ 'class' => 'apihelp-block apihelp-permissions' ]
        );
        $m = $this->msg('api-help-permissions');
        if (!$m->isDisabled()) {
            $help['permissions'] .= Html::rawElement(
                'div',
                [ 'class' => 'apihelp-block-head' ],
        ...
        if (empty($options['nolead'])) {
            ...
            $help['datatypes'] .= Html::rawElement(
                'h' . min(6, $level),
