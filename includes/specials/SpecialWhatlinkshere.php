...
class SpecialWhatLinksHere extends IncludableSpecialPage
{
     ...
     */
    public function showIndirectLinks($level, $target, $limit, $from = 0, $back = 0)
    {
        $out = $this->getOutput();
    ...
    public function makeSelfLink($text, $query)
    {
    ...
    public function getPrevNext($prevId, $nextId)
    {
    ...
    public function whatlinkshereForm()
    {
    ...
    public function getFilterPanel()
    {
        ...
        foreach ($types as $type) {
            ...
            $links[] = $this->msg("whatlinkshere-{$type}")->rawParams(
                $this->makeSelfLink($msg, array_merge($changed, $overrides))
            )->escaped();
        }
