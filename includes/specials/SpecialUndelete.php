...
$trace = debug_backtrace();
    ...
    private function undeleteRevisions($timestamps, $unsuppress = false, $comment = '')
    {
        ...
        if ($page) {
            ...
            $previousTimestamp = $dbw->selectField(
                'revision',
                'rev_timestamp',
...
class SpecialUndelete extends SpecialPage
{
    ...
    public function loadRequest($par)
    {
    ...
    private function showRevision($timestamp)
    {
        ...
        $out->addHTML($this->msg('undelete-revision')->rawParams($link)->params(
            $time
        )->rawParams($userLink)->params($d, $t)->parse() . '</div>');
     ...
     * @return string HTML
     */
    public function showDiff($previousRev, $currentRev)
    {
            ...
            if ($this->getUser()->isAllowed('suppressrevision')) {
                $unsuppressBox =
                    "<tr>
						...
						<td class='mw-input'>" .
                        Xml::checkLabel(
                            $this->msg('revdelete-unsuppress')->text(),
                            'wpUnsuppress',
     ...
     * @return string
     */
    public function getPageLink($rev, $titleObj, $ts)
    {
        $user = $this->getUser();
     ...
     */
    public function getFileLink($file, $titleObj, $ts, $key)
    {
        $user = $this->getUser();
        $time = $this->getLanguage()->userTimeAndDate($ts, $user);
     ...
     * @return string HTML fragment
     */
    public function getFileUser($file)
    {
     ...
     * @return string HTML fragment
     */
    public function getFileComment($file)
    {
