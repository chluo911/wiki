...
abstract class LoginSignupSpecialPage extends AuthManagerSpecialPage
{
    ...
    protected function getPageHtml($formHtml)
    {
        ...
        $loginPrompt = $this->isSignup() ? '' : Html::rawElement(
            'div',
            [ 'id' => 'userloginprompt' ],
            $this->msg('loginprompt')->parseAsBlock()
        );
        ...
        if ($this->isSignup() && $this->showExtraInformation()) {
            ...
            for ($benefitIdx = 1; $benefitIdx <= $benefitCount; $benefitIdx++) {
                ...
                $benefitList .= Html::rawElement(
                    'div',
                    [ 'class' => "mw-number-text $iconClass" ],
                    Html::rawElement(
                        'h3',
                        [],
        ...
        if ($this->mEntryError) {
            $fieldDefinitions['entryError'] = [
                ...
                'default' => Html::rawElement(
                    'div',
                    [ 'class' => $this->mEntryErrorType . 'box', ],
                    $this->mEntryError
                ),
                'raw' => true,
                'rawrow' => true,
