...
class Parser
{
    ...
    public $mConf;
    public $mExtLinkBracketedRegex;
    public $mUrlProtocols;
    ...
    public function parse(
        ...
        if ($this->mOptions->getEnableLimitReport()) {
            ...
            $limitReport = str_replace([ '-', '&' ], [ 'â€?', '&amp;' ], $limitReport);
            $text .= "\n<!-- \n$limitReport-->\n";
...
$trace = debug_backtrace();
    ...
    public function replaceInternalLinks2(&$s)
    {
        ...
        for (; $line !== false && $line !== null; $a->next(), $line = $a->current()) {
                ...
                if ($ns == NS_FILE) {
                    if (!wfIsBadImage($nt->getDBkey(), $this->mTitle)) {
                        ...
                        $s .= $prefix . $this->armorLinks(
                            $this->makeImage($nt, $text, $holders)
                        ) . $trail;
                        continue;
                    }
            ...
            if ($ns == NS_MEDIA) {
                ...
                $s .= $prefix . $this->armorLinks(
                    Linker::makeMediaLinkFile($nt, $file, $text)
                ) . $trail;
                continue;
            }
    ...
    protected function makeKnownLinkHolder($nt, $text = '', $trail = '', $prefix = '')
    {
        ...
        $link = $this->getLinkRenderer()->makeKnownLink(
            $nt,
            new HtmlArmor("$prefix$text$inside")
        );
...
$trace = debug_backtrace();
    ...
    private function pstPass2($text, $user)
    {
        ...
        // [[ns:page (context)|]]
        $p1 = "/\[\[(:?$nc+:|:|)($tc+?)( ?\\($tc+\\))\\|]]/";
        // [[ns:pageï¼ˆcontextï¼‰|]] (double-width brackets, added in r40257)
        $p4 = "/\[\[(:?$nc+:|:|)($tc+?)( ?ï¼?$tc+ï¼?)\\|]]/";
     ...
     * @param bool $clearState
     */
    public function startExternalParse(
        Title $title = null,
        ParserOptions $options,
        $outputType,
        $clearState = true
