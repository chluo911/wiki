...
$trace = debug_backtrace();
     ...
     */
    public function addMetadataLink(array $linkarr)
    {
$trace = debug_backtrace();
	  error_log(__FILE__);
	  var_dump(__FUNCTION__);
...
$trace = debug_backtrace();
     ...
     * @param bool $linestart Is this the start of a line?
     */
    public function addWikiTextTitleTidy($text, &$title, $linestart = true)
    {
     ...
     */
    public function addWikiTextTitle(
        $text,
        Title $title,
        $linestart,
        $tidy = false,
...
$trace = debug_backtrace();
     ...
     * @param ParserOutput $parserOutput
     */
    public function addParserOutput($parserOutput)
    {
...
$trace = debug_backtrace();
     ...
     */
    public function addAcceptLanguage()
    {
        $title = $this->getTitle();
...
$trace = debug_backtrace();
    ...
    public function addReturnTo($title, array $query = [], $text = null, $options = [])
    {
        $link = $this->msg('returnto')->rawParams(
            Linker::link($title, $text, [], $query, $options)
        )->escaped();
    ...
    public function getHeadLinksArray()
    {
        ...
        if (!$config->get('DisableLangConversion')) {
            ...
            if ($lang->hasVariants()) {
                $variants = $lang->getVariants();
                foreach ($variants as $variant) {
                    $tags["variant-$variant"] = Html::element(
                        'link',
                        [
                        'rel' => 'alternate',
